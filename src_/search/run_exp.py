...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        if args.train and run_manager.best_acc == 0:
            loss, acc1, acc5 = run_manager.validate(is_test=False, return_top5=False)
    ...
    if run_config.valid_size:
        ...
        #loss, acc1, acc5 = run_manager.validate(is_test=False, return_top5=False)
        loss, acc1, f1score, precision, recall = run_manager.validate(is_test=False, return_top5=False)
        #log = 'valid_loss: %f\t valid_acc1: %f\t valid_acc5: %f' % (loss, acc1, acc5)
        log = 'valid_loss: %f\t valid_acc1: %f\t valid_f1score: %f\t valid_precision: %f\t valid_recall: %f:' % (loss, acc1, f1score, precision, recall)
        ...
        output_dict = {
            **output_dict,
            'valid_loss': ' % f' % loss, 'valid_acc1': ' % f' % acc1, 'valid_f1score': ' % f' % f1score, 'valid_precision': ' % f' % precision, 'valid_recall': ' % f' % recall,
            'valid_size': run_config.valid_size
        }

    # test
    print('Test model on test set')
    #loss, acc1, acc5 = run_manager.validate(is_test=True, return_top5=False)
    #log = 'test_loss: %f\t test_acc1: %f\t test_acc5: %f' % (loss, acc1, acc5)
    loss, acc1, f1score, precision, recall = run_manager.validate(is_test=True, return_top5=False)
    log = 'test_loss: %f\t test_acc1: %f\t test_f1score: %f\t test_precision: %f\t test_recall: %f' % (loss, acc1, f1score, precision, recall)
    ...
    output_dict = {
        **output_dict,
        'test_loss': '%f' % loss, 'test_acc1': '%f' % acc1, 'test_f1score': '%f' % f1score,  'test_precision': ' % f' % precision, 'test_recall': ' % f' % recall
    }
    ...
    if args.quantize:
        ...
        for n_bits in bits_per_run:
            ...
            if run_config.valid_size:
                ...
                loss, acc1, f1score, precision, recall = run_manager.validate(is_test=False, return_top5=False)
                log = '%d bit quant valid_loss: %f\t valid_acc1: %f\t valid_f1score: %f\t valid_precision: %f\t valid_recall: %f' % (n_bits, loss, acc1, f1score, precision, recall)
                run_manager.write_log(log, prefix='valid')
                # output_dict = {
                #     **output_dict,
                #     'valid_loss': ' % f' % loss, 'valid_acc1': ' % f' % acc1, 'valid_acc5': ' % f' % acc5,
            ...
            loss, acc1, f1score, precision, recall = run_manager.validate(is_test=True, return_top5=False)
            log = '%d bit quant test_loss: %f\t test_acc1: %f\t test_f1score: %f\t test_precision: %f\t test_recall: %f' % (n_bits, loss, acc1, f1score, precision, recall)
            run_manager.write_log(log, prefix='test')
            # output_dict = {
            #     **output_dict,
            #     'test_loss': '%f' % loss, 'test_acc1': '%f' % acc1, 'test_acc5': '%f' % acc5
